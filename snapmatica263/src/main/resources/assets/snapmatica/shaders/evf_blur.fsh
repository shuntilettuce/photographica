...
uniform float DepthM22;          // projection z row: distance = DepthM32 / (ndc + DepthM22)
uniform float DepthM32;
uniform int   DepthClip01;       // 1 = clip depth 0..1 (glClipControl), 0 = -1..1
uniform int   DepthReversed;     // 1 = far plane at depth 0, nearer = LARGER depth
...
// Off the z row of the projection that drew the frame, so it holds in whichever depth
// convention that was -- on 26.3 it flips with the shader toggle (EvfBlurRenderer.depthM22).
float linearDepth(float d) {
    float ndc  = (DepthClip01 == 1) ? d : 2.0 * d - 1.0;
    float dist = DepthM32 / (ndc + DepthM22);
    return (dist > 0.0) ? min(dist, 1.0e6) : 1.0e6;   // infinite far plane: sky, not NaN
}
...
bool drawnAfterDepthCopy(vec2 uv) {
    ...
    return (DepthReversed == 1) ? live > copied + 1e-5 : live < copied - 1e-5;
}
