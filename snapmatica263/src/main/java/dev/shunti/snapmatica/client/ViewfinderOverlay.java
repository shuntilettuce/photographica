...
public final class ViewfinderOverlay {
    ...
    // Fabric's HudRenderCallback passes a DeltaTracker from 1.21 on; 1.20.1's still
    ...
    *///?}
        ...
        if (ApertureIntegration.isActive()) {
            ...
            ctx.text(mc.font, msg, (sw - tw) / 2, sh / 2 - 12, 0xFFB0B0B0, false);
        ...
        ctx.text(tr, String.format("F%s  %s  ISO%d  %s%s",
        ...
        if (SnapmaticaClient.lensType != 0) {
            ...
            ctx.text(tr, fd, fx2 - tr.width(fd) - 6, fy + 4, rc);
        }
        ...
        String lens = hasLens
                ...
                : Component.translatable("snapmatica.vf.no_lens").getString();
        ctx.text(tr, Component.literal(lens), fx+6, fy+4, 0xFF9A8D72);
        ...
        ctx.text(tr,Component.literal(
                ...
                fx+6,fy+4+tr.lineHeight+2,0xFFCCCCFF);
     ...
     */
    private static void renderLevel(GuiGraphicsExtractor ctx, Font tr, int fx, int fy, int fx2, int fy2) {
        Minecraft mc = Minecraft.getInstance();
        ...
        ctx.text(tr, label, cx + half + 16, cy - tr.lineHeight / 2, color);
    }

    private static void drawTurnedLine(GuiGraphicsExtractor ctx, int cx, int cy, int half, double angle, int color) {
    ...
    private static void drawAfPointGrid(GuiGraphicsExtractor ctx, int cx, int cy, int color) {
    ...
    private static void drawAfBox(GuiGraphicsExtractor ctx, int x, int y, int color) {
