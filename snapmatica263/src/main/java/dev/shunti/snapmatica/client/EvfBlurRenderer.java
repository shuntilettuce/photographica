...
public final class EvfBlurRenderer {
    ...
    private static int peakLocDepthM22  = -1;
    private static int peakLocDepthM32  = -1;
    private static int peakLocDepthClip01 = -1;
    ...
    /** GL internal format depthTex was allocated in; the source's, see captureDepth. */
    private static int depthTexFormat = 0;
    ...
    private static int locDepthM22   = -1;
    private static int locDepthM32   = -1;
    private static int locDepthClip01 = -1;
    private static int locDepthReversed = -1;
    ...
    /**
     * How a depth-buffer sample turns back into a distance: the z row of the projection that
     * drew the frame, and the clip range OpenGL rasterised it into.
     *
     * <p>26.3 has no single depth convention, and which one is live changes while the game runs:
     * <ul>
     *   <li>Vanilla builds every projection REVERSED -- near and far swapped, so the far plane
     *       and the cleared sky sit at window depth 0 -- and turns clip control (0..1) on where
     *       the driver offers it.
     *   <li>Iris turns clip control off at startup, pack or no pack. With Iris installed and
     *       shaders OFF the projection is therefore reversed over -1..1.
     *   <li>With a shader pack ON, Iris swaps near and far back while the world is extracted and
     *       drawn, so the pack sees the classic forward -1..1 it was written for.
     * </ul>
     * Toggling shaders in the menu moves between the last two. So nothing here assumes one. The
     * distance is D = m32 / (ndc + m22) off the matrix that drew the frame, which is exact in
     * every combination of forward/reversed and -1..1/0..1 (round-tripped through JOML's own
     * matrices in all four), and the clip range is asked of OpenGL itself rather than of the
     * device info, which Iris edits.
     */
    private static float   depthM22    = -1.0002f;   // forward -1..1, near 0.05 far 512, until the first frame
    private static float   depthM32    = -0.1000f;
    private static boolean depthClip01 = false;
    /** The far plane is at window depth 0 and nearer surfaces have LARGER depth values. */
    public static boolean  depthReversed = false;

    /** OpenGL's actual clip depth range: 0..1 under glClipControl, else the classic -1..1. */
    private static boolean glClipZeroToOne() {
        try {
            org.lwjgl.opengl.GLCapabilities caps = org.lwjgl.opengl.GL.getCapabilities();
            if (!caps.GL_ARB_clip_control && !caps.OpenGL45) return false;
            return GL11.glGetInteger(org.lwjgl.opengl.GL45C.GL_CLIP_DEPTH_MODE)
                    == org.lwjgl.opengl.GL45C.GL_ZERO_TO_ONE;
        } catch (Throwable t) {
            return false;
        }
    }

    /** A depth-buffer sample as a distance in blocks, in whichever convention drew the frame. */
    public static float linearizeDepth(float d) {
        float ndc = depthClip01 ? d : 2.0f * d - 1.0f;
        float dist = depthM32 / (ndc + depthM22);
        // An infinite far plane puts the sky at a zero denominator: +-Infinity or NaN, never a
        // distance. Read it as "very far", which is what it is.
        return (dist > 0.0f) ? Math.min(dist, 1.0e6f) : 1.0e6f;
    }

    /** The far plane, which is also what the sky is cleared to. */
    public static boolean isFarPlaneDepth(float d) {
        return depthReversed ? d <= 0.000001f : d >= 0.999999f;
    }

    /** Hard against the near plane: nothing real is drawn there, so an empty or garbage read. */
    public static boolean isNearPlaneDepth(float d) {
        return depthReversed ? d >= 0.999f : d <= 0.001f;
    }

    /** Distance at one end of the window depth range; +Infinity for an infinite far plane. */
    private static float endDistance(float ndc, float m22, float m32) {
        float v = m32 / (ndc + m22);
        return (Float.isFinite(v) && v > 0.0f) ? v : Float.POSITIVE_INFINITY;
    }

    /**
     * Derives the TRUE far plane that generated the depth buffer and stores it in
     * {@link #currentDepthFar}, along with the rest of the depth mapping (see {@link #depthM22}).
     * The heuristic this replaced (renderDistance * 64) ignored
     ...
     * <ol>
     *   <li>{@code projection} — the matrix actually used for the world render.</li>
     *   <li>{@code gameFarPlane} — a direct read of the value vanilla feeds into that matrix.</li>
    ...
    public static void updateDepthFar(org.joml.Matrix4f projection, float gameFarPlane,
                                      float fallbackFar) {
        final boolean clip01 = glClipZeroToOne();
        float far = -1.0f;
        boolean fromMatrix = false;
        if (projection != null) {
            try {
                float m22 = projection.m22(), m32 = projection.m32();
                // Which end of the depth range is the far one IS the convention, so read both
                // ends off the matrix instead of deciding in advance.
                float atLow  = endDistance(clip01 ? 0.0f : -1.0f, m22, m32);
                float atHigh = endDistance(1.0f, m22, m32);
                float pn = Math.min(atLow, atHigh);
                float pf = Math.max(atLow, atHigh);
                // The near plane matters more than it looks: every distance is divided through
                // it. Vanilla's is 0.05, but a shader pipeline is free to change it.
                if (pn > 0.0f && pn < 16.0f) {
                    depthM22 = m22;
                    depthM32 = m32;
                    depthClip01 = clip01;
                    depthReversed = atLow > atHigh;
                    currentDepthNear = pn;
                    fromMatrix = true;
        ...
        if (!fromMatrix) {
            // No usable matrix: the classic forward mapping for these planes, as before.
            org.joml.Matrix4f p = new org.joml.Matrix4f()
                    .setPerspective(1.0f, 1.0f, currentDepthNear, far, clip01);
            depthM22 = p.m22();
            depthM32 = p.m32();
            depthClip01 = clip01;
            depthReversed = false;
        ...
        // far-plane extension (or the failure to see it), and a shader toggle flipping the
        // convention, visible without a debug build.
        int convention = (depthReversed ? 2 : 0) | (depthClip01 ? 1 : 0);
        if (lastLoggedFar < 0.0f || far < lastLoggedFar * 0.9f || far > lastLoggedFar * 1.1f
                || convention != lastLoggedConvention) {
            lastLoggedFar = far;
            lastLoggedConvention = convention;
            System.out.println("[Snapmatica] depth planes: near=" + currentDepthNear
                    + "  far=" + far + " blocks  (" + (depthReversed ? "reversed" : "forward")
                    + ", clip " + (depthClip01 ? "0..1" : "-1..1") + ")");
        }
        currentDepthFar = far;
    }

    private static int lastLoggedConvention = -1;
    ...
    public static void captureDepth(int fbW, int fbH) {
        ...
        com.mojang.renderpearl.api.textures.GpuTexture depthGpu_ = mainFb_.getDepthTexture();
        if (!(depthGpu_ instanceof com.mojang.renderpearl.backend.opengl.GlTexture glDepth_)) return;
        ...
        // The copy has to be in the source's OWN internal format, so ask the source rather than
        // assume one. glCopyImageSubData wants depth formats to match exactly, and a mismatch
        // does not necessarily fail loudly: 26.3 moved the main target from fixed-point DEPTH32
        // to D32_FLOAT, and copying that into the DEPTH32 this used to allocate raised no GL
        // error at all on NVIDIA -- it just wrote wrong values (0.0123 came out as 0.2872,
        // measured on the driver). The blur then worked from a nonsense depth, so the focus
        // plane no longer sat where the rangefinder, which raycasts, said it was.
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, srcDepthId_);
        int srcFormat_ = GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0,
                GL11.GL_TEXTURE_INTERNAL_FORMAT);
        if (srcFormat_ == 0) {   // no storage behind the id this frame: nothing to copy
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, prevTex2D_);
            GL13.glActiveTexture(prevActiveTU_);
            return;
        }
        if (depthTex == -1 || depthTexW != fw_ || depthTexH != fh_ || depthTexFormat != srcFormat_) {
            if (depthTex != -1) GL11.glDeleteTextures(depthTex);
            depthTex = GL11.glGenTextures();
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, depthTex);
            // Immutable storage takes the internal format alone -- no pixel format/type pair to
            // get right for each depth or depth-stencil variant. Resizes recreate the texture.
            org.lwjgl.opengl.GL42.glTexStorage2D(GL11.GL_TEXTURE_2D, 1, srcFormat_, fw_, fh_);
            depthTexFormat = srcFormat_;
    ...
    public static void renderBlur(int fx, int fy, int fx2, int fy2,
        ...
        //? if >=1.21.10 {
        com.mojang.renderpearl.api.textures.GpuTexture gpuTex = mainFb.getColorTexture();
        if (!(gpuTex instanceof com.mojang.renderpearl.backend.opengl.GlTexture glTex)) return;
        ...
        *///?}
        ...
        int liveDepthTex = (mainFb.getDepthTexture()
                instanceof com.mojang.renderpearl.backend.opengl.GlTexture glDepthLive)
                ? glDepthLive.glId() : 0;
        //?} else {
        /*int liveDepthTex = mainFb.getDepthTexture();
        *///?}
        ...
        if (locVignette >= 0) {
            ...
            int vgSw = Math.max(1, mc.getWindow().getGuiScaledWidth());
            int vgSh = Math.max(1, mc.getWindow().getGuiScaledHeight());
        ...
        if (locAfPoint >= 0) {
            int afSw = Math.max(1, mc.getWindow().getGuiScaledWidth());
            int afSh = Math.max(1, mc.getWindow().getGuiScaledHeight());
        ...
        GL20.glUniform1f(locDepthM22, depthM22);
        GL20.glUniform1f(locDepthM32, depthM32);
        GL20.glUniform1i(locDepthClip01, depthClip01 ? 1 : 0);
        GL20.glUniform1i(locDepthReversed, depthReversed ? 1 : 0);
    ...
    private static void applyPeaking(int mainTex, int fbW, int fbH, float focusDist,
        ...
        GL20.glUniform1f(peakLocDepthM22, depthM22);
        GL20.glUniform1f(peakLocDepthM32, depthM32);
        GL20.glUniform1i(peakLocDepthClip01, depthClip01 ? 1 : 0);
    ...
    public static float readCenterLinearDepthBlocks() {
        ...
        // "Nearest" compared as a distance: which way the raw value runs is the depth
        // convention, and on 26.3 that changes with the shader toggle (see depthM22).
        float nearest = Float.POSITIVE_INFINITY;
        for (int i = 0; i < N * N; i++) {
            float d = buf.get(i);
            if (isNearPlaneDepth(d) || isFarPlaneDepth(d)) continue;   // empty read / sky
            float dist = linearizeDepth(d);
            if (dist > 0.0f && dist < nearest) nearest = dist;
        }
        return (nearest < Float.POSITIVE_INFINITY) ? nearest : SnapmaticaClient.FOCUS_INFINITY;
    }
    //?}
    ...
    public static void applyBlur(boolean forCapture, boolean captureHQ) {
        ...
        boolean viewfinderUp = SnapmaticaClient.viewfinderActive(mc) && mc.gui.screen() == null;
    ...
    public static float[] readLinearDepthCpu() {
        ...
        for (int i = 0; i < linear.length; i++) {
            linear[i] = linearizeDepth(buf.get(i));
        }
        return linear;
    }
    ...
    private static void initProgram() {
        try {
            ...
            locDepthM22  = GL20.glGetUniformLocation(program, "DepthM22");
            locDepthM32  = GL20.glGetUniformLocation(program, "DepthM32");
            locDepthClip01 = GL20.glGetUniformLocation(program, "DepthClip01");
            locDepthReversed = GL20.glGetUniformLocation(program, "DepthReversed");
    ...
    private static void initPeakProgram() {
        try {
            ...
            peakLocDepthM22  = GL20.glGetUniformLocation(peakProgram, "DepthM22");
            peakLocDepthM32  = GL20.glGetUniformLocation(peakProgram, "DepthM32");
            peakLocDepthClip01 = GL20.glGetUniformLocation(peakProgram, "DepthClip01");
