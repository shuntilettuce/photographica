...
public final class Freecam {
     ...
     * (player look turning) while freecam is active so the player's own facing never moves.
     * Scaling matches the vanilla look path exactly, so freecam turns at the same rate normal
     * look does.
     *
     ...
     */
    public static void addPathKeyframe(Minecraft client) {
        ...
        if (client.player == null) return;
        if (dist < 0) {
            client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.keyframe_first", path.size()), true);
        } else {
            client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.keyframe_added",
     ...
     */
    public static void deleteTargetedKeyframe(Minecraft client) {
        if (!active || pathPlaying) return;
        ...
        if (client.player != null) {
            client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.keyframe_removed", path.size()), true);
        }
    }

    /** Drops every recorded keyframe and stops any playback in progress. */
    public static void clearPath(Minecraft client) {
        path.clear();
        stopPathPlayback();
        if (client.player != null) {
            client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.cleared"), true);
        }
    }
     ...
     */
    public static void togglePathPlayback(Minecraft client) {
        if (!active) return;
        if (pathPlaying) {
            stopPathPlayback();
            return;
        ...
        if (path.size() < 2) {
            if (client.player != null) {
                client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.too_short"), true);
            }
            return;
        }
        pinActive = false;
     ...
     */
    public static void playPathWithRecording(Minecraft client) {
        if (!active || pathPlaying) return;
     ...
     */
    public static void insertKeyframeOnCurve(Minecraft client) {
        ...
        if (client.player != null) {
            client.gui.hud.setOverlayMessage(Component.translatable("snapmatica.path.keyframe_inserted", path.size()), true);
        }
    }
    ...
    /**
     * Mouse-wheel input while a keyframe is being dragged, moving it along the look direction
     * instead of the scroll wheel's usual zoom/aperture/etc. job: scroll up pushes it away,
     * scroll down pulls it closer.
     *
     ...
     */
    public static void togglePin(Minecraft client) {
        ...
        for (Entity e : client.level.getEntities(client.player, searchBox, Entity::isAlive)) {
        ...
        // getEntities always excludes the entity passed as its first argument — normally
        ...
        Vec3 toSelf = client.player.getBoundingBox().getCenter().subtract(pos);
    ...
    public static void tick(Minecraft client) {
        ...
        boolean w = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_W);
        boolean s = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_S);
        boolean a = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_A);
        boolean d = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_D);
        boolean up = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_SPACE);
        boolean down = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_LSHIFT);
        boolean fast = InputConstants.isKeyDown(com.mojang.blaze3d.platform.InputConstants.KEY_LCONTROL);
