...
public final class PhotoCapture {
    ...
    public static void captureIfPending() {
        ...
        //? if >=1.21.10 {
        Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget(), raw -> processScreenshot(mc, raw, capturedDepth, capturedFbW, capturedFbH, fmt));
        //?} else {
        /*NativeImage raw;
        try {
            raw = Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget());
        } catch (Exception e) {
    ...
    private static void tickAccumulation() {
        ...
        if (now >= accumNextMs && accumSamples < ACCUM_MAX_SAMPLES) {
            //? if >=1.21.10 {
            Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget(), PhotoCapture::accumulateFrame);
            //?} else {
            /*try {
                accumulateFrame(Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget()));
            } catch (Exception e) {
     ...
     * {@link AutoFocus#raycastThroughGlass(Minecraft, Vec3, Vec3, double, boolean)}.
     */
    static double nearestSubjectDistance(Minecraft mc, net.minecraft.world.phys.Vec3 eye,
                                         float baseYaw, float basePitch, double maxDist,
