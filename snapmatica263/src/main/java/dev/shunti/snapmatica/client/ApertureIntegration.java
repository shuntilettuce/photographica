...
public final class ApertureIntegration {
    ...
    public static void arm(int photoFormat, double shutterSec) {
        ...
        heldWorldTime = (mcNow != null && mcNow.level != null) ? mcNow.level.getOverworldClockTime() : -1L;
        ...
        if (!SnapmaticaClient.apertureDebugSamples && mcNow != null) {
            int fw = mcNow.gameRenderer.mainRenderTarget().width, fh = mcNow.gameRenderer.mainRenderTarget().height;
    ...
    public static void tick(Minecraft mc) {
        ...
        if (issued < total && !inFlight && now >= nextMs) {
            ...
            //? if >=1.21.10 {
            long t0 = System.nanoTime();
            Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget(), ApertureIntegration::accumulate);
            readbackNanos += System.nanoTime() - t0;
            //?} else {
            /*try {
                accumulate(Screenshot.takeScreenshot(mc.gameRenderer.mainRenderTarget()));
            } catch (Exception e) {
