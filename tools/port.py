...
# Every method name the JDK and JOML define, extracted with javap into
# jdk_names.txt. A call whose receiver's type is unknown -- `.name(` with
# anything in front -- is never rewritten if the JDK or JOML already uses that
# name: yarn keeps adding methods, and each release turned more of the mod's own
# JDK calls into Minecraft ones. 1.21.11 alone produced Class.getMethod ->
# getMethodName, List.toArray -> adjustArgs and Matrix4f.perspective ->
# cameraType. A denylist written by hand after each surprise was always one
# release behind; this one is the JDK's own list. Calls on a known Minecraft
# class (Camera.getYaw and the like) still go through the owner-qualified pass.
def _jdk_names():
    p = os.path.join(HERE, "jdk_names.txt")
    if not os.path.isfile(p):
        return set()
    return {l.strip() for l in io.open(p, encoding="utf-8") if l.strip()}


JDK_METHOD_NAMES = _jdk_names()
...
def rewrite_members(text):
    ...
    def global_sub(m):
        name = m.group(1)
        if name in PROTECTED or name in JDK_MEMBERS or name in JDK_METHOD_NAMES:
            return m.group(0)
