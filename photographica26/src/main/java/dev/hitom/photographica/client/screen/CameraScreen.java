...
public class CameraScreen extends Screen {
    ...
    public CameraScreen(ItemStack stack, int armorStandEntityId) {
        super(Component.translatable("screen.photographica.camera"));
        this.stack = stack;
    ...
    protected void init() {
        ...
        if (armorStandEntityId >= 0) {
            ...
            if (!sdData.isEmpty()) {
                addRenderableWidget(SafelightButton.primary(cx - 105, btnY, 100,
                        Component.literal("SDカード (" + sdData.photos().size() + ")"),
                        b -> {
                            if (dirty) {
                ...
                addRenderableWidget(SafelightButton.ghost(cx - 50, btnY, 100,
                        Component.literal("閉じる"), b -> onClose()));
            }
        }
    }
