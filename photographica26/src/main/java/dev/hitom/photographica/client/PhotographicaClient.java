...
public class PhotographicaClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			// Tripod recording films from the stand via a render-only camera swap, but
			// the camera ENTITY must stay the player every tick or movement, look, sneak
			// and interaction input all die.  Assert it here as a safety net — a cheap
			// no-op when already correct — so the camera can never get stuck on the stand.
			...
			if (standId >= 0 && client.level != null
					&& client.level.getEntity(standId) == null) {
