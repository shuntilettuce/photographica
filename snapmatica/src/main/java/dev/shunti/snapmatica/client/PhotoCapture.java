...
public final class PhotoCapture {
    ...
    public static void captureIfPending() {
        ...
        if (ApertureIntegration.isActive()) { ApertureIntegration.tick(MinecraftClient.getInstance()); return; }
        ...
        //? if >=1.21.10 {
        ScreenshotRecorder.takeScreenshot(mc.getFramebuffer(), raw -> processScreenshot(mc, raw, capturedDepth, capturedFbW, capturedFbH, fmt));
        //?} else {
        /*NativeImage raw;
        try {
     ...
     */
    static void deliverIntegrated(MinecraftClient mc, NativeImage img, int format) {
        processScreenshot(mc, img, null, 0, 0, format);
    }

    private static void processScreenshot(MinecraftClient mc, NativeImage raw, float[] linearDepth, int fbW, int fbH, int format) {
     ...
     */
    static org.joml.Matrix4f worldProjection(MinecraftClient mc) {
    ...
    static double nearestSubjectDistance(MinecraftClient mc, net.minecraft.util.math.Vec3d eye,
        ...
        for (float[] o : offsets) {
            ...
            net.minecraft.util.math.Box searchBox =
                    new net.minecraft.util.math.Box(eye, eye).stretch(look.multiply(maxDist)).expand(1.0);
            ...
            if (Freecam.isActive()) {
                java.util.Optional<net.minecraft.util.math.Vec3d> playerHit = mc.player
                        .getBoundingBox().expand(mc.player.getTargetingMargin()).raycast(eye, end);
