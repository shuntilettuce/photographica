...
public final class EntityExposure {
     ...
     */
    public static void recordCameraForPendingSlots(net.minecraft.client.MinecraftClient mc) {
        ...
        for (float[] p : pendingSlots()) {
            ...
            double[] pose;
            if (Freecam.isActive()) {
                net.minecraft.util.math.Vec3d fp =
                ...
                pose = new double[] { fp.x, fp.y, fp.z,
                        net.minecraft.util.math.MathHelper.lerpAngleDegrees(
                                phi, Freecam.getPrevYaw(), Freecam.getYaw()),
                        net.minecraft.util.math.MathHelper.lerp(
                                phi, Freecam.getPrevPitch(), Freecam.getPitch()) };
            } else {
                //? if >=1.21.2 {
                net.minecraft.util.math.Vec3d cp = mc.player.getCameraPosVec(phi);
                pose = new double[] { cp.x, cp.y, cp.z,
                        mc.player.getLerpedYaw(phi), mc.player.getLerpedPitch(phi) };
                //?} else {
                /*net.minecraft.util.math.Vec3d cp = mc.player.getCameraPosVec(phi);
                pose = new double[] { cp.x, cp.y, cp.z,
                        mc.player.prevYaw + (mc.player.getYaw() - mc.player.prevYaw) * phi,
                        mc.player.prevPitch + (mc.player.getPitch() - mc.player.prevPitch) * phi };
                *///?}
            }
