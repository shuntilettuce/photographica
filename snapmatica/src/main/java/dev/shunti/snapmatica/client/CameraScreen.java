...
public class CameraScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(CameraUi.Button.ghost(cx + 4, by, 80,
                Text.translatable(Freecam.isActive()
                        ? "snapmatica.common.exit_freecam" : "snapmatica.common.freecam"),
                b -> { if (client != null) { Freecam.toggle(client); close(); } }));
    }
    ...
    *///?}
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("snapmatica.camera.tab_photo"),
                colLeftX, headerY, CameraUi.CREAM_DIM);
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("snapmatica.camera.tab_camera"),
        ...
        for (Heading h : headings) {
            String label = Text.translatable(h.key()).getString();
            int halfW = textRenderer.getWidth(label) / 2;
            int ruleY = h.y() + 4;
