 ...
 * read ASYNCHRONOUSLY through a pixel buffer. The sample is added a frame later, when its mean
 * has arrived. Two staging slots let the next sample be decoded while the previous one waits.
 *
...
final class BurstAccumulator {
    ...
    private static final int[] stageTex = {-1, -1};
    private static final int[] stageFbo = {-1, -1};
    private static final int[] pbo = {-1, -1};
    private static final long[] fence = {0L, 0L};
    private static final boolean[] pending = {false, false};
    ...
    static void begin(int fbW, int fbH) {
        ensureReady(fbW, fbH);
        for (int i = 0; i < 2; i++) { dropFence(i); pending[i] = false; }
        slot = 0;
    ...
    static void submit(MinecraftClient mc, Sink sink) {
        ...
        State st = State.save();
        try {
            // The other slot is one frame old: its mean is (almost always) already here.
            int other = slot ^ 1;
            if (pending[other]) fold(other, sink, true);

            // Decode this frame into the free slot, average it down, and ask for the mean.
            drawInto(stageFbo[slot], mainTex, 0, 1f, false);
            GL11.glBindTexture(GL11.GL_TEXTURE_2D, stageTex[slot]);
    ...
    static void flush(Sink sink) {
        ...
        try {
            ...
            int a = slot, b = slot ^ 1;
            if (pending[a]) fold(a, sink, true);
            if (pending[b]) fold(b, sink, true);
        } finally {
            st.restore();
        }
    }
    ...
    static boolean hasPending() { return pending[0] || pending[1]; }
    ...
    private static void fold(int s, Sink sink, boolean block) {
        if (fence[s] != 0L) {
            GL32.glClientWaitSync(fence[s], GL32.GL_SYNC_FLUSH_COMMANDS_BIT,
                    block ? 1_000_000_000L : 0L);
            dropFence(s);
        }
