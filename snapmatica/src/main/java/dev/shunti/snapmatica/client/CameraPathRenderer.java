 ...
 * version used {@code RenderLayers.lines()} directly in the world, which worked fine under the
 ...
 *
 * <p>The view and projection are computed fresh every frame from {@code Camera.getRotation()}
 * and the same focal-length FOV formula {@code GameRendererMixin} uses — an earlier version
 * captured Mojang's own matrices via a mixin on {@code WorldRenderer.render()} instead, which
...
public final class CameraPathRenderer {
    ...
    public static void render(net.minecraft.client.gui.DrawContext ctx,
        ...
        double aspect = (double) mc.getWindow().getFramebufferWidth()
        ...
        if (path.size() >= 2) {
            for (int i = 0; i < path.size() - 1; i++) {
                ...
                for (int s = 1; s <= CURVE_SEGMENTS_PER_SPAN; s++) {
                    ...
                    Vec3d cur = catmullRom(p0.pos(), p1.pos(), p2.pos(), p3.pos(), t);
                    boolean blocked = mc.world != null && segmentBlocked(mc.world, prev, cur);
    ...
    private static void drawIndex(net.minecraft.client.gui.DrawContext ctx, net.minecraft.client.MinecraftClient mc,
                                  Vec3d camPos, org.joml.Quaternionf view, org.joml.Matrix4f proj,
                                  int sw, int sh, Vec3d pos, int number) {
    ...
    private static void drawFrustum(net.minecraft.client.gui.DrawContext ctx, Vec3d camPos,
        ...
        double halfAngleH = Math.atan(SENSOR_HALF_W / crop / focal);
        double halfAngleV = Math.atan(SENSOR_HALF_H / crop / focal);

        Vec3d[] near = rect(kf.pos(), forward, right, up, FRUSTUM_NEAR,
                Math.tan(halfAngleH) * FRUSTUM_NEAR, Math.tan(halfAngleV) * FRUSTUM_NEAR);
        Vec3d[] far = rect(kf.pos(), forward, right, up, FRUSTUM_FAR,
    ...
    private static Vec3d[] rect(Vec3d origin, Vec3d forward, Vec3d right, Vec3d up,
                                double dist, double halfW, double halfH) {
        Vec3d center = origin.add(forward.multiply(dist));
        return new Vec3d[]{
    ...
    private static void drawWorldSegment(net.minecraft.client.gui.DrawContext ctx, Vec3d camPos,
                                         org.joml.Quaternionf view, org.joml.Matrix4f proj, int sw, int sh,
                                         Vec3d a, Vec3d b, int color) {
    ...
    private static boolean segmentBlocked(net.minecraft.client.world.ClientWorld world, Vec3d a, Vec3d b) {
        net.minecraft.util.hit.BlockHitResult hit = world.raycast(new net.minecraft.world.RaycastContext(
                a, b, net.minecraft.world.RaycastContext.ShapeType.COLLIDER,
    ...
    private static double[] worldToScreen(Vec3d pos, Vec3d camPos, org.joml.Quaternionf view,
    ...
    /** DrawContext has no arbitrary-angle line primitive, only axis-aligned fills — this spins
     *  its 2D matrix stack to the segment's own angle and fills one straight rectangle in that
     *  rotated space, rather than walking the segment and stamping many small squares along it.
     *  One fill() instead of dozens per segment (a real cost at typical keyframe counts — the
     *  earlier dot-chain was the actual source of the slowdown), and a clean straight edge
     *  instead of a visibly chunky dotted one. */
    private static void stampLine(net.minecraft.client.gui.DrawContext ctx,
