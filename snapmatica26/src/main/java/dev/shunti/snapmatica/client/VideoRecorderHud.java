...
/**
 * HUD overlay for video recording status (MC 26.1.2).
 *
...
public final class VideoRecorderHud {
    ...
    private static void renderRecording(GuiGraphicsExtractor ctx, Minecraft mc) {
        ...
        int w1 = font.width(recLabel);
        int w2 = font.width(timeLabel);
        int bannerW = padding * 2 + w1 + 8 + w2;
        int x = sw - bannerW - 8;

        ctx.fill(x - 2, y - 2, x + bannerW, y + 10, BG_DARK);
        ctx.text(font, recLabel,  x + padding,          y, RED_BRIGHT, false);
        ctx.text(font, timeLabel, x + padding + w1 + 8, y, timeColor,  false);
    }
    ...
    private static void renderPostProcessing(GuiGraphicsExtractor ctx, Minecraft mc) {
        ...
        ctx.centeredText(font, Component.literal("⚙ " + msg), sw / 2, panelY + 4, CREAM);
