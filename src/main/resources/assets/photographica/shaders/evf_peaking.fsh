...
//       (and the screenshot, on the rare frame this runs just before one — guarded against by
//       the caller never scheduling peaking for a capture frame) expects the finished frame.

uniform sampler2D InSampler;
uniform sampler2D DepthSampler;  // non-linear depth [0,1] from the scene framebuffer; pass 0 only
uniform int   Pass;
uniform vec2  PixelSize;
uniform float FocusDist;         // focus distance in blocks — already resolved (MF/AF/MOB) in Java
uniform float Near;
uniform float Far;
...
void main() {
    ...
    float band = max(FocusDist * 0.03, 0.1);
    if (abs(depth - FocusDist) > band) { fragColor = vec4(srcColor, 1.0); return; }
