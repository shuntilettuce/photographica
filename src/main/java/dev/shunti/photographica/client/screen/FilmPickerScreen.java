...
public class FilmPickerScreen extends Screen {
    ...
    protected void init() {
        ...
        for (int ft : types) {
            ...
            int finalFt = ft;
            addDrawableChild(SafelightButton.primary(bx, y, bw,
                    Text.literal(label),
                    b -> {
        ...
        y += 6;
        addDrawableChild(SafelightButton.ghost(bx, y, bw,
                Text.literal("キャンセル"),
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer,
                Text.literal("フィルム選択"), width / 2, py + 6, GuiHelper.CREAM);
