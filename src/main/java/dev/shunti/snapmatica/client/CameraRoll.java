...
public final class CameraRoll {
     ...
     */
    public static boolean onButton(Minecraft mc, int button, boolean pressed) {
        if (button == 0) leftHeld = pressed;
        if (button != 1) return false;
        rightHeld = pressed;
        if (pressed) {
            if (mc.screen != null || !inCameraContext(mc)) return false;
    ...
    public static void releaseIfStale(Minecraft mc) {
        if (gripping && (mc.screen != null || !rightHeld || !inCameraContext(mc))) {
            gripping = false;
            SnapmaticaConfig.save();
        }
    }
