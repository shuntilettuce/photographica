...
public final class DhIntegration {
    ...
    private static boolean checkPresent() {
        return net.minecraftforge.fml.ModList.get().isLoaded("distanthorizons");
    }

    /** Returns distance in blocks to the first DH terrain along the look ray, or -1f if none. */
    public static float queryLookDistance(Minecraft mc) {
    ...
    private static float queryInternal(Minecraft mc) {
