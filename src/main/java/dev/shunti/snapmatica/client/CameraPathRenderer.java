 ...
 * version used {@code RenderTypes.lines()} directly in the world, which worked fine under the
 ...
 * captured Mojang's own matrices via a mixin on {@code LevelRenderer.renderLevel()} instead, which
 ...
 *
 * <p>Ported below 1.21.6: PoseStack in place of the 1.21.6+ Matrix3x2fStack.
 */
...
public final class CameraPathRenderer {
    ...
    public static void render(net.minecraft.client.gui.GuiGraphics ctx,
                              float partialTick) {
        ...
        if (VideoRecorder.isRecording() || SnapmaticaClient.lensType != 0) {
            float focal = Freecam.currentFocalLengthMm(partialTick);
        ...
        if (path.size() >= 2) {
            for (int i = 0; i < path.size() - 1; i++) {
                ...
                for (int s = 1; s <= CURVE_SEGMENTS_PER_SPAN; s++) {
                    ...
                    boolean blocked = mc.level != null && segmentBlocked(mc.level, prev, cur);
    ...
    private static void drawIndex(net.minecraft.client.gui.GuiGraphics ctx, net.minecraft.client.Minecraft mc,
        ...
        ctx.drawCenteredString(mc.font, label,
                (int) Math.round(p[0]), (int) Math.round(p[1]) - 14, 0xFFFFFFFF);
    }

    private static void drawFrustum(net.minecraft.client.gui.GuiGraphics ctx, Vec3 camPos,
    ...
    private static void drawWorldSegment(net.minecraft.client.gui.GuiGraphics ctx, Vec3 camPos,
                                         org.joml.Quaternionf view, org.joml.Matrix4f proj, int sw, int sh,
    ...
    private static boolean segmentBlocked(net.minecraft.client.multiplayer.ClientLevel world, Vec3 a, Vec3 b) {
    ...
    /** Level point → screen pixel: relative-to-camera position, rotated into view space by the
    ...
    /** GuiGraphics has no arbitrary-angle line primitive, only axis-aligned fills — this spins
    ...
    private static void stampLine(net.minecraft.client.gui.GuiGraphics ctx,
        ...
        m.mulPose(com.mojang.math.Axis.ZP.rotation(angle));
