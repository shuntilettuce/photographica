 ...
 *
 * <p>Stopping the tick loop is not on the table: a minecraft mod that stops simulating desyncs the
 ...
 * minecraft tick, the render frame that follows it asks vanilla for the states at the phases
 * belonging to that tick, and keeps them. Condition the last one is in, the EXPOSE phase runs the
...
public final class EntityExposure {
    ...
    /** One minecraft tick has completed; the next render frame can record the slots it carries. */
    ...
    public static void endSnapshotFrame() {
        ...
        // Recording advances on minecraft ticks, and a minecraft that stops ticking — a pause menu in
     ...
     * quantities {@code Camera.setup} interpolates, asked for at the phase that matters.
     *
        ...
        for (float[] p : pendingSlots()) {
            ...
            if (Freecam.isActive()) {
                ...
                pose = new double[] { cp.x, cp.y, cp.z,
                        mc.player.yRotO + (mc.player.getYRot() - mc.player.yRotO) * phi,
                        mc.player.xRotO + (mc.player.getXRot() - mc.player.xRotO) * phi };
            }
