...
public final class ViewfinderOverlay {
    ...
    public static void render(GuiGraphics ctx, DeltaTracker tickCounter) {
        ...
        if (ApertureIntegration.isActive()) {
            ...
            ctx.drawString(mc.font, msg, (sw - tw) / 2, sh / 2 - 12, 0xFFB0B0B0, false);
        ...
        if (SnapmaticaClient.lensType != 0) {
            ...
            ctx.drawString(tr, fd, fx2 - tr.width(fd) - 6, fy + 4, rc);
        }
        ...
        String lens = hasLens
                ...
                : Component.translatable("snapmatica.vf.no_lens").getString();
        ctx.drawString(tr, Component.literal(lens), fx+6, fy+4, 0xFF9A8D72);
        ...
        ctx.drawString(tr,Component.literal(
     ...
     *
     * <p>RecordedDeviation from the fixed neutral reference (f/5.6, 1/60, ISO 400) — how far the DIAL
    ...
    private static void drawAfPointGrid(GuiGraphics ctx, int cx, int cy, int color) {
     ...
     */
    private static void drawHomeMark(GuiGraphics ctx, int x, int y) {
    ...
    private static void drawAfBox(GuiGraphics ctx, int x, int y, int color) {
