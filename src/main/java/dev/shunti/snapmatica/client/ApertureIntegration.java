...
public final class ApertureIntegration {
     ...
     * <ul>
     *   <li>{@code Level.getDayTime} — the sun, the moon, the sky colour, and (through
     ...
     * a minecraft mod does not get to stop the tick loop. What holds them together instead is the
     ...
     *                     real time, so entities — which the tick loop owns and a minecraft mod
    ...
    public static void arm(int photoFormat, double shutterSec) {
        ...
        heldWorldTime = (mcNow != null && mcNow.level != null) ? mcNow.level.getDayTime() : -1L;
    ...
    public static void tick(Minecraft mc) {
        ...
        // recorded, one minecraft tick at a time, and the burst cannot start until it has the
        ...
        if (issued < total && !inFlight && now >= nextMs) {
            ...
            long t0 = System.nanoTime();
            Screenshot.takeScreenshot(mc.getMainRenderTarget(), ApertureIntegration::accumulate);
    ...
    public static void shear(Matrix4f proj) {
        ...
        // Camera.move in CameraMixin, which is the same thing walking does and which every
