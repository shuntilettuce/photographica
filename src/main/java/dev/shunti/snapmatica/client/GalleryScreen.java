...
public class GalleryScreen extends Screen {
     ...
     */
    private CameraUi.SafelightButton copyBtn, revealBtn, openBtn, deleteBtn, backBtn, colsBtn;
    ...
    private String captionFor(long modified, int avail) {
        ...
        for (SimpleDateFormat f : STAMPS) {
            String t = f.format(d);
            if (font.width(t) <= avail) return t;
        }
        return "";
    }
    ...
    private void renderGrid(GuiGraphics ctx, int mouseX, int mouseY) {
        ctx.drawString(font, Component.translatable("snapmatica.gallery.title"), PAD + 2, 10, 0xFFE8DCC4);
        String count = Component.translatable("snapmatica.gallery.items", entries.size()).getString();
        ctx.drawString(font, Component.literal(count),
                width - PAD - font.width(count) - 2, 10, 0xFF7A7A85);
        ...
        if (entries.isEmpty()) {
            ctx.drawCenteredString(font,
                    Component.translatable("snapmatica.gallery.empty"), width / 2, height / 2, 0xFF7A7A85);
            return;
        }
        ...
        ctx.drawCenteredString(font,
                Component.translatable("snapmatica.gallery.help_grid"),
                width / 2, height - FOOTER_H + 3, CameraUi.CREAM_DIM);
    }

    private void drawCell(GuiGraphics ctx, MediaLibrary.Entry e, int x, int y, int w, int h, boolean hover) {
        int thumbH = w * 2 / 3;
        ...
        if (tex != null) {
            ...
            ctx.drawCenteredString(font,
                    e.video() ? Component.translatable("snapmatica.gallery.video") : Component.literal("..."),
        ...
        if (e.video()) {
            ...
            ctx.drawString(font, Component.literal("▶"), bx, by, 0xFFFFFFFF);
        }
        ...
        if (!stamp.isEmpty()) {
            ctx.drawString(font, Component.literal(stamp), x + 1, y + thumbH + 3, 0xFF8A8A95);
        }
    }

    private void renderViewer(GuiGraphics ctx) {
        MediaLibrary.Entry e = entries.get(viewing);
        ...
        ctx.drawString(font, Component.literal(e.displayName()), PAD + 2, 10, 0xFFE8DCC4);
        String pos = (viewing + 1) + " / " + entries.size();
        ctx.drawString(font, Component.literal(pos),
                width - PAD - font.width(pos) - 2, 10, 0xFF7A7A85);

        drawExifPanel(ctx, exifLines, exifW, top, bottom);

        ctx.drawCenteredString(font,
                Component.translatable("snapmatica.gallery.help_viewer"),
                width / 2, height - FOOTER_H + 3, CameraUi.CREAM_DIM);
    }
    ...
    private void drawExifPanel(GuiGraphics ctx, java.util.List<String> lines, int panelW,
                               int top, int bottom) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            ctx.drawString(font, Component.literal(lines.get(i)), px + 6, ty,
    ...
    private void drawFitted(GuiGraphics ctx, Identifier tex, int bx, int by, int bw, int bh, float ar) {
        int dw = bw, dh = Math.round(bw / ar);
        if (dh > bh) { dh = bh; dw = Math.round(bh * ar); }
        int dx = bx + (bw - dw) / 2, dy = by + (bh - dh) / 2;
        // Three generations, three signatures: a RenderPipeline in 1.21.11, a RenderType
    ...
    /**
     * 1.21.11 replaced the loose (x, y, button) / (key, scancode, mods) parameters with MouseButtonEvent
     * and KeyEvent records, so each version gets its own thin wrapper over the shared logic.
