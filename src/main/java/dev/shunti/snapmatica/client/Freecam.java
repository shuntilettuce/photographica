 ...
 * camera: while {@link #isActive()}, it cancels {@code Camera.setup()} and applies the
 ...
 *
 * <p>FrontAndTop is tracked independently of the player (see {@link #onMouseLook}, fed by
...
public final class Freecam {
    ...
    /** FrontAndTop as of the previous tick, for interpolating orbit's auto-facing — see {@link #getPrevYaw}. */
    private static float prevYaw, prevPitch;

    /** The player's real {@link ClientInput} while freecam is off, to restore on exit. */
    ...
    private static Vec3 pinPos = Vec3.ZERO;
     ...
     */
    public record Keyframe(Vec3 pos, float yaw, float pitch, int focalLengthMm,
                           boolean pinned, Vec3 pinTarget) {}
    ...
    private static Vec3 pathFocusLockPoint = Vec3.ZERO;
    ...
    private static Vec3 targetedCurvePos = Vec3.ZERO;
    ...
    public static void toggle(Minecraft minecraft) {
        ...
        if (active) deactivate(minecraft);
        else activate(minecraft);
    }
    ...
    private static void deactivate(Minecraft minecraft) {
        ...
        stopPathPlayback();
        if (minecraft.player != null && savedInput != null) {
            minecraft.player.input = savedInput;
        }
        savedInput = null;
    }
     ...
     */
    public static void toggleLock(Minecraft minecraft) {
        if (!active || minecraft.player == null) return;
        locked = !locked;
        if (locked) {
            if (savedInput != null) minecraft.player.input = savedInput;
        } else {
            minecraft.player.input = new ClientInput();
        }
    }
     ...
     * moves. GuiSpriteScaling matches {@code Entity.turn} exactly, so freecam turns at
    ...
    public static void onMouseLook(double dx, double dy) {
        ...
        yaw += (float) (dx * 0.15);
        pitch = Mth.clamp((float) (pitch + dy * 0.15), -90.0f, 90.0f);
    }
    ...
    public static void addPathKeyframe(Minecraft minecraft) {
        ...
        if (minecraft.player == null) return;
        if (dist < 0) {
            minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.keyframe_first", path.size()), true);
        } else {
            minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.keyframe_added",
     ...
     */
    public static void deleteTargetedKeyframe(Minecraft minecraft) {
        if (!active || pathPlaying) return;
        ...
        if (minecraft.player != null) {
            minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.keyframe_removed", path.size()), true);
        }
    }

    /** Drops every recorded keyframe and stops any playback in progress. */
    public static void clearPath(Minecraft minecraft) {
        path.clear();
        stopPathPlayback();
        if (minecraft.player != null) {
            minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.cleared"), true);
        }
    }
     ...
     */
    public static void togglePathPlayback(Minecraft minecraft) {
        if (!active) return;
        if (pathPlaying) {
            stopPathPlayback();
            return;
        ...
        if (path.size() < 2) {
            if (minecraft.player != null) {
                minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.too_short"), true);
            }
            return;
        }
        pinActive = false;
     ...
     */
    public static void playPathWithRecording(Minecraft minecraft) {
        if (!active || pathPlaying) return;
    ...
    private static void updateTargetedKeyframe() {
        ...
        for (int i = 0; i < path.size(); i++) {
            Vec3 toKf = path.get(i).pos().subtract(pos);
            double dist = toKf.length();
            if (dist < 0.1 || dist > bestDist) continue;
            if (toKf.normalize().dot(look) >= KEYFRAME_CONE_COS) {
                best = i;
                bestDist = dist;
            }
        }
    ...
    private static void updateTargetedCurvePoint() {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            for (int s = 1; s < CURVE_TEST_SAMPLES; s++) {
                ...
                Vec3 pt = catmullRom(p0.pos(), p1.pos(), p2.pos(), p3.pos(), t);
                Vec3 toPt = pt.subtract(pos);
                double dist = toPt.length();
                if (dist < 0.1 || dist > bestDist) continue;
                if (toPt.normalize().dot(look) >= KEYFRAME_CONE_COS) {
                    bestSeg = i;
                    bestDist = dist;
                    bestPos = pt;
                }
     ...
     * after "on a keyframe" (delete) and "nothing targeted" (append at the end). FrontAndTop
     * and lens come from the current view, same as a normal append; only the position is the
     * point that was actually aimed at.
     */
    public static void insertKeyframeOnCurve(Minecraft minecraft) {
        ...
        if (minecraft.player != null) {
            minecraft.player.displayClientMessage(Component.translatable("snapmatica.path.keyframe_inserted", path.size()), true);
        }
    }
    ...
    private static Vec3 savedFlyPos = Vec3.ZERO;
    ...
    private static void updateKeyframeDragState() {
        ...
        Vec3 newPos = pos.add(lookDir().scale(dragDepth));
    ...
    /**
     * MouseHandler-look while {@link #isEditingKeyframeOrientation()} — adjusts the dragged
    ...
    public static void onMouseLookForKeyframe(double dx, double dy) {
        ...
        float newPitch = Mth.clamp((float) (old.pitch() + dy * 0.15), -90.0f, 90.0f);
    ...
    /**
     * MouseHandler-wheel input while a keyframe is being dragged: normally moves it along the look
     ...
     */
    public static void togglePin(Minecraft minecraft) {
        if (!active || minecraft.level == null) return;
        ...
        Vec3 end = pos.add(look.scale(maxDist));
        BlockHitResult blockHit = AutoFocus.raycastThroughGlass(minecraft, pos, look, maxDist);
        double bestDist = (blockHit != null && blockHit.getType() != HitResult.Type.MISS)
                ? pos.distanceTo(blockHit.getLocation()) : maxDist;
        Vec3 hitPos = (blockHit != null && blockHit.getType() != HitResult.Type.MISS)
                ? blockHit.getLocation() : end;

        Entity nearest = null;
        double nearestDist = bestDist;
        AABB searchBox = new AABB(pos, pos).inflate(maxDist);
        for (Entity e : minecraft.level.getEntities(minecraft.player, searchBox,
                ent -> ent.isAlive())) {
            Vec3 toEnt = e.getBoundingBox().getCenter().subtract(pos);
            double dist = toEnt.length();
            if (dist < 0.1 || dist > nearestDist) continue;
            if (toEnt.normalize().dot(look) >= PIN_CONE_COS) {
                nearest = e;
                nearestDist = dist;
            }
        }
        ...
        Vec3 toSelf = minecraft.player.getBoundingBox().getCenter().subtract(pos);
        double selfDist = toSelf.length();
        if (selfDist >= 0.1 && selfDist <= nearestDist
                && toSelf.normalize().dot(look) >= PIN_CONE_COS) {
            nearest = minecraft.player;
        ...
        orbitRadius = Math.max(ORBIT_MIN_RADIUS, fromCenter.horizontalDistance());
     ...
     */
    public static void togglePathFocusLock(Minecraft minecraft) {
    ...
    public static void tick(Minecraft minecraft) {
        ...
        if (hp < lastHealth) {
            // InteractionHand control back the instant something hits the player — frozen and facing
        ...
        boolean up = InputConstants.isKeyDown(window, GLFW.GLFW_KEY_SPACE);
        boolean down = InputConstants.isKeyDown(window, GLFW.GLFW_KEY_LEFT_SHIFT);
        boolean fast = InputConstants.isKeyDown(window, GLFW.GLFW_KEY_LEFT_CONTROL);
    ...
    private static void tickDirect(boolean w, boolean s, boolean a, boolean d,
        ...
        if (move.lengthSqr() > 0) {
            pos = pos.add(move.normalize().scale(speed));
        }
    }
    ...
    private static void tickInertia(boolean w, boolean s, boolean a, boolean d,
        ...
        if (thrust.lengthSqr() > 0) {
            velocity = velocity.add(thrust.normalize().scale(accelMag));
        }
        ...
        velocity = velocity.scale(DRONE_DRAG);
    ...
    private static void tickOrbit(boolean w, boolean s, boolean a, boolean d,
        ...
        Vec3 center = (pinEntity != null) ? pinEntity.getBoundingBox().getCenter() : pinPos;
    ...
    private static void tickPath() {
        ...
        if (kf1.pinned() && kf2.pinned()) {
            Vec3 target = kf1.pinTarget().lerp(kf2.pinTarget(), localT);
    ...
    /** AbstractUniform Catmull-Rom (tension 0.5) through p1→p2, using p0/p3 as the neighbours that
