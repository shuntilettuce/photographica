...
public final class CameraUi {
    ...
    public static class SafelightButton extends Button {
        ...
        protected void renderContents(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
            drawBody(ctx);
            renderDefaultLabel(ctx.textRendererForWidget(this, GuiGraphics.HoveredTextEffects.NONE));
        }

        @Override
        protected void renderDefaultLabel(net.minecraft.client.gui.ActiveTextCollector consumer) {
            net.minecraft.network.chat.Component label = getMessage().copy().withStyle(s -> s.withColor(
                    net.minecraft.network.chat.TextColor.fromRgb(labelColour() & 0xFFFFFF)));
            renderScrollingStringOverContents(consumer, label, 2);
        }
    }
}
