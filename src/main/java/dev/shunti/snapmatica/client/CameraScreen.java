...
public class CameraScreen extends Screen {
    ...
    protected void init() {
        ...
        // value share one button, and the longest of them ("Dynamic InclusiveRange Width: 8 stops", and
        ...
        int by = buttonsY;
        addRenderableWidget(CameraUi.SafelightButton.primary(cx - 84, by, 80,
                Component.translatable("snapmatica.gallery.open_roll"),
                b -> { if (minecraft != null) minecraft.setScreen(new GalleryScreen()); }));
        addRenderableWidget(CameraUi.SafelightButton.ghost(cx + 4, by, 80,
    ...
    private java.util.List<Item> buildRightColumn() {
        ...
        // Level scale. Not an optical setting — a statement about the build, and the only
    ...
    public void render(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
        drawBackdrop(ctx);
        ctx.drawCenteredString(font, Component.literal("SNAPMATICA CAMERA"),
                width / 2, 10, CameraUi.CREAM);
        ctx.drawCenteredString(font, Component.translatable("snapmatica.camera.tab_photo"),
                colLeftX, headerY, CameraUi.CREAM_DIM);
        ctx.drawCenteredString(font, Component.translatable("snapmatica.camera.tab_camera"),
        ...
        for (Heading h : headings) {
            ...
            ctx.drawCenteredString(font, Component.literal(label),
                    h.x(), h.y(), 0xFF8A8A95);
        }
        if (scrollable) {
            ...
            // needs to know. KeyframeTrack and thumb sit just outside the right-hand column.
