...
public class SnapmaticaClient {
    ...
    /**
     * Condition true, freecam does not force the player's body into view — see
    ...
    /** Condition true, sneaking shows the viewfinder overlay (default: enabled). */
    ...
    /** Called from the mod entrypoint's RegisterKeyMappingsEvent listener. */
    public static void registerKeyMappings(net.neoforged.neoforge.client.event.RegisterKeyMappingsEvent event) {
        shootKey = register(event, new KeyMapping(
                "key.snapmatica.shoot", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_ENTER, "category.snapmatica"));
        ...
        pinKey = register(event, new KeyMapping(
                "key.snapmatica.pin", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_X, "category.snapmatica"));
        freecamLockKey = register(event, new KeyMapping(
                "key.snapmatica.freecam_lock", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_C, "category.snapmatica"));
        pathMenuKey = register(event, new KeyMapping(
                "key.snapmatica.path_menu", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_Z, "category.snapmatica"));
    }

    private static KeyMapping register(net.neoforged.neoforge.client.event.RegisterKeyMappingsEvent event, KeyMapping key) {
        event.register(key);
        return key;
    }
    ...
    public static void onRenderGui(GuiGraphics ctx, float partialTick) {
    ...
    /**
     * NeoForge RenderLevelStageEvent AFTER_TRIPWIRE_BLOCKS — fires right before the
     * translucent pass, which is where Fabric's BEFORE_TRANSLUCENT / BEFORE_DEBUG_RENDER
     * fired. The depth copy happens before translucent so glass cannot stamp its own
     * surface distance over the view through it.
     */
    public static void onBeforeTranslucent(net.minecraft.client.Camera camera, float partialTick) {
        PhotoCapture.onBeforeTranslucent();
    }

    /** NeoForge RenderLevelStageEvent AFTER_LEVEL — end of the world render pass. */
    public static void onWorldRenderEnd(net.minecraft.client.Camera camera, float partialTick) {
    ...
    private static void updateAmbientFocus(Minecraft mc) {
        ...
        if (now - ambientFocusQueryMs >= 100L) {
            ...
            double d = PhotoCapture.nearestSubjectDistance(mc, eye,
                    camera.getYRot(), camera.getXRot(), maxDist, new float[][]{{0, 0}}, true);
     ...
     *
     * Condition {@link #dynamicRangeSim} is on, that reference point itself shifts by
