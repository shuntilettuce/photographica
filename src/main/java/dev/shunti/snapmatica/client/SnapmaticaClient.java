...
public class SnapmaticaClient {
    ...
    private static KeyMapping register(net.minecraftforge.client.event.RegisterKeyMappingsEvent event, KeyMapping key) {
        event.register(key);
        return key;
    }

    /** End of every Minecraft tick. Called from the entrypoint's ClientTickEvent listener. */
    ...
    /**
     * HUD overlay (viewfinder, blackout, flash, video REC). Called from Forge's
     * RenderGuiOverlayEvent.Post, which is the 1.20.1 stand-in for Fabric's HudRenderCallback.
     */
    public static void onRenderGui(GuiGraphics ctx, float partialTick) {
        ViewfinderOverlay.render(ctx, partialTick);
        VideoRecorderHud.render(ctx, partialTick);
        FreecamHud.render(ctx, partialTick);
        CameraPathRenderer.render(ctx, partialTick);
    }

    /**
     * Forge RenderLevelStageEvent AFTER_TRIPWIRE_BLOCKS — the point right before the
     * translucent pass, which is where Fabric's BEFORE_DEBUG_RENDER fired. The depth copy
     * happens before translucent so glass cannot stamp its own surface distance over the
     * view through it.
     */
    ...
    /** Forge RenderLevelStageEvent AFTER_LEVEL — end of the world render pass. */
