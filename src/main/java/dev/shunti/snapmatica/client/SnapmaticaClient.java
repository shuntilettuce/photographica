...
public class SnapmaticaClient {
    ...
    private static KeyMapping pathMenuKey;          // camera path: open the add/play/clear/duration menu
    // ── Camera state (minecraft-side only, no server sync needed) ───────────────────
     ...
     * {@code Camera.detached = true} specifically so vanilla's "don't render the entity the
    ...
    /**
     * Called from the entrypoint's RegisterKeyMappingsEvent listener. 1.21.9 gave key
     * categories an object of their own, and NeoForge wants it registered before the keys
     * that name it -- otherwise the controls screen has nowhere to put them.
     */
    public static void registerKeyMappings(net.neoforged.neoforge.client.event.RegisterKeyMappingsEvent event) {
        event.registerCategory(SNAPMATICA_CATEGORY);
        shootKey = register(event, new KeyMapping(
                "key.snapmatica.shoot", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_ENTER, SNAPMATICA_CATEGORY));
        settingsKey = register(event, new KeyMapping(
                "key.snapmatica.settings", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_G, SNAPMATICA_CATEGORY));
        viewfinderSneakKey = register(event, new KeyMapping(
                "key.snapmatica.viewfinder_sneak", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_COMMA, SNAPMATICA_CATEGORY));
        recordKey = register(event, new KeyMapping(
                "key.snapmatica.record", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_R, SNAPMATICA_CATEGORY));
        pinKey = register(event, new KeyMapping(
                "key.snapmatica.pin", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_X, SNAPMATICA_CATEGORY));
        freecamLockKey = register(event, new KeyMapping(
                "key.snapmatica.freecam_lock", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_C, SNAPMATICA_CATEGORY));
        pathMenuKey = register(event, new KeyMapping(
                "key.snapmatica.path_menu", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_Z, SNAPMATICA_CATEGORY));
    }

    private static KeyMapping register(net.neoforged.neoforge.client.event.RegisterKeyMappingsEvent event,
                                       KeyMapping key) {
        event.register(key);
        return key;
    }

    /** End of every Minecraft tick. Called from the entrypoint's ClientTickEvent.Post listener. */
    public static void onClientTick() {
        Minecraft minecraft = Minecraft.getInstance();
        // Before the early return: the exposure recorder counts minecraft ticks, and it has to
        ...
        while (recordKey.consumeClick()) {
            ...
            else if (!VideoRecorder.isPostProcessing()) minecraft.setScreen(new VideoRecorderScreen());
        }
        ...
        while (pinKey.consumeClick()) {
            Freecam.togglePin(minecraft);
        }
        while (freecamLockKey.consumeClick()) {
            Freecam.toggleLock(minecraft);
        }
        while (pathMenuKey.consumeClick()) {
            if (minecraft.screen instanceof CameraPathScreen) {
                minecraft.setScreen(null);
            } else if (Freecam.isActive() && minecraft.screen == null) {
                minecraft.setScreen(new CameraPathScreen());
            }
        }
    ...
    /** HUD overlay (viewfinder, blackout, flash, video REC). Called from RenderGuiEvent.Post. */
    public static void onRenderGui(net.minecraft.client.gui.GuiGraphics ctx,
                                   net.minecraft.client.DeltaTracker delta) {
        ViewfinderOverlay.render(ctx, delta);
        VideoRecorderHud.render(ctx, delta);
        FreecamHud.render(ctx, delta);
        CameraPathRenderer.render(ctx, delta);
    }

    /**
     * RenderLevelStageEvent.AfterEntities -- the last stage NeoForge 21.11 posts before the
     * translucent pass, which is where Fabric's BEFORE_TRANSLUCENT fires. The depth copy has
     * to happen here so glass cannot stamp its own surface distance over the view through it.
     * (AfterTripwireBlocks, which the older NeoForge ports used, comes AFTER translucent.)
     */
    public static void onBeforeTranslucent() {
        PhotoCapture.onBeforeTranslucent();
    }

    /** RenderLevelStageEvent.AfterLevel -- end of the world render pass. */
     ...
     */
    private static void updateMetering(Minecraft mc) {
        if (mc.level == null) { meteredExtraStops = 0.0; return; }
        ...
        net.minecraft.world.phys.Vec3 eye = cameraPos(mc);
        ...
        for (int i = 0; i < offsets.length; i++) {
            ...
            if (hit != null && hit.getType() != net.minecraft.world.phys.HitResult.Type.MISS) {
                net.minecraft.core.BlockPos lit = hit.getBlockPos().relative(hit.getDirection());
                light      = mc.level.getMaxLocalRawBrightness(lit);
                blockLight = mc.level.getBrightness(net.minecraft.world.level.LightLayer.BLOCK, lit);
                skyLight   = mc.level.getBrightness(net.minecraft.world.level.LightLayer.SKY, lit);
            } else {
     ...
     */
    private static void updateWhiteBalance(Minecraft mc, double blockLight, double skyLight) {
        ...
        // have the light already half gone by mid-afternoon. GuiSpriteScaling the cosine steeply and
    ...
    public static float[] whiteBalanceGain() {
        ...
        // and a twisted hue. Ease keeps the direction of the correction and only gives up its
     ...
     * moves. Ease linearly in blocks would crawl up close and snap in the distance.
     */
    private static void updateAmbientFocus(Minecraft mc) {
        if (!ambientDof || mc.level == null) return;
    ...
    public static net.minecraft.world.phys.Vec3 cameraLook(Minecraft mc) {
        net.minecraft.client.Camera camera = mc.gameRenderer.getMainCamera();
        return net.minecraft.world.phys.Vec3.directionFromRotation(camera.xRot(), camera.yRot());
    }
