...
public final class ClipboardUtil {
    ...
    public static void copyImageAsync(File imageFile) {
        copyFileReferenceAsync(imageFile, Component.translatable("snapmatica.clip.photo"));
    }

    /** Copy a saved file (e.g. an MP4) to the clipboard as a file reference (async). */
    public static void copyFileAsync(File file) {
        copyFileReferenceAsync(file, Component.translatable("snapmatica.clip.video"));
    }

    private static void copyFileReferenceAsync(File file, Component successMsg) {
    ...
    private static void run(String name, ClipboardTask task, Component successMsg) {
        Thread t = new Thread(() -> {
            try {
                task.run();
                ...
                e.printStackTrace();
                actionBar(Component.translatable("snapmatica.clip.failed", e.getClass().getSimpleName()));
            }
        }, name);
        t.setDaemon(true);
        t.start();
    ...
    private static void actionBar(Component msg) {
