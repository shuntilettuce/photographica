...
/**
 * Two-pass separable Gaussian blur rendered entirely on the GPU.
 * Used for the mirrorless camera EVF depth-of-field preview.
 *
 * Pass 1 (H): main framebuffer colour → auxFbo (horizontal Gaussian)
 * Pass 2 (V): auxFbo → main framebuffer (vertical Gaussian, scissored to the
 *             viewfinder frame so only that region is overwritten)
 *
 * All GL state that is modified is saved before and restored after.
 */
...
public final class EvfBlurRenderer {
    ...
    /**
     * Applies a two-pass Gaussian blur to the viewfinder frame area of the
     * current draw framebuffer. fx/fy/fx2/fy2 are in scaled GUI coordinates.
     * blurRadius is in framebuffer pixels.
     */
    public static void renderBlur(int fx, int fy, int fx2, int fy2, float blurRadius) {
        ...
        ensureInit(fbW, fbH);
        if (program == -1) return; // init failed
        ...
        GL20.glUniform1f(locRadius, blurRadius);
        ...
        int scY = fbH - (int)(fy2 * scale);   // GL Y=0 is bottom; fy2 is bottom of GUI frame
    ...
    private static void initProgram() {
        try {
            ...
            locRadius    = GL20.glGetUniformLocation(program, "Radius");
            ...
            // UV(0,0) = bottom-left of GL framebuffer = bottom of screen
            float[] verts = {
                -1f, -1f,  0f, 0f,
                 1f, -1f,  1f, 0f,
                -1f,  1f,  0f, 1f,
