...
public final class VideoRecorder {
    ...
    /**
     * True only while the off-screen tripod render pass is executing.
     * During this window the depth read in onWorldRenderEnd() targets the
     * off-screen framebuffer, and WorldRendererMixin un-skips the player so
     * the operator appears in frame.
     */
    ...
    public static void startRecording(ItemStack stack, int armorStandEntityId) {
        ...
        // Free-view tripod (all versions): no camera switch here.  The tripod view is
        // rendered off-screen per-frame in captureTripodFrame() while the player keeps
    ...
    public static void stopRecording() {
        ...
        // Release the off-screen tripod framebuffer.
        if (tripodFb != null) { tripodFb.delete(); tripodFb = null; }
    ...
    public static void onWorldRenderEnd() {
        ...
        // image).  Skip every WorldRenderEvents.LAST/END_MAIN that isn't the tripod pass.
    ...
    public static void captureFrameIfRecording() {
        if (!recording) return;
        // Tripod frames are captured off-screen in captureTripodFrame() — skip here.
    ...
    // ── ensureTripodFb: API differs per version; written as chained guards to avoid
    //    nesting /*  */ blocks inside a parent commented-out block.
    //? if >=1.21.11 {
    /*private static void ensureTripodFb(int w, int h) {
        if (tripodFb == null) tripodFb = new net.minecraft.client.gl.SimpleFramebuffer("photographica_tripod", w, h, true);
        else if (tripodFb.textureWidth != w || tripodFb.textureHeight != h) tripodFb.resize(w, h);
    }*/
    //?} else if >=1.21.4 {
    /*private static void ensureTripodFb(int w, int h) {
        if (tripodFb == null) tripodFb = new net.minecraft.client.gl.SimpleFramebuffer(w, h, true);
        else if (tripodFb.textureWidth != w || tripodFb.textureHeight != h) tripodFb.resize(w, h);
        tripodFb.setClearColor(0f, 0f, 0f, 1f); tripodFb.clear(); tripodFb.beginWrite(true);
    }*/
    //?} else {
    private static void ensureTripodFb(int w, int h) {
        if (tripodFb == null) tripodFb = new net.minecraft.client.gl.SimpleFramebuffer(w, h, true, MinecraftClient.IS_SYSTEM_MAC);
        else if (tripodFb.textureWidth != w || tripodFb.textureHeight != h) tripodFb.resize(w, h, MinecraftClient.IS_SYSTEM_MAC);
        tripodFb.setClearColor(0f, 0f, 0f, 1f); tripodFb.clear(MinecraftClient.IS_SYSTEM_MAC); tripodFb.beginWrite(true);
    }
    //?}

    // ── captureTripodFrame ─────────────────────────────────────────────────────
    // 1.21.11 version: BEFORE-renderWorld inject; FrameGraph uses mc.getFramebuffer()
    //                  as "main" node, so swapping mc.framebuffer redirects the pass.
    //? if >=1.21.11 {
    /*public static void captureTripodFrame(RenderTickCounter tickCounter) {
        ...
        dev.hitom.photographica.mixin.client.MinecraftClientAccessor acc =
                (dev.hitom.photographica.mixin.client.MinecraftClientAccessor) mc;

        ensureTripodFb(w, h);

        net.minecraft.entity.Entity prevCam = acc.photographica$getCameraEntityDirect();
        try {
            acc.photographica$setCameraEntityDirect(stand);
            mc.gameRenderer.updateCamera(tickCounter);
            acc.photographica$setFramebuffer(tripodFb);

            tripodRenderPass = true;
            mc.gameRenderer.renderWorld(tickCounter);
            tripodRenderPass = false;
        } catch (Exception e) {
            tripodRenderPass = false;
            Photographica.LOGGER.warn("[VideoRecorder] Tripod frame {} failed", frameCount, e);
        } finally {
            acc.photographica$setFramebuffer(mainFb);
            acc.photographica$setCameraEntityDirect(prevCam != null ? prevCam : mc.player);
            mc.gameRenderer.updateCamera(tickCounter);
        }

        float[] depthGrid;
        if (pendingDepthReady && pendingDepthGrid != null) {
            depthGrid = pendingDepthGrid; pendingDepthGrid = null; pendingDepthReady = false;
        } else {
            depthGrid = flatDepthGrid(currentFocusDepth);
        }
        ...
        float ap = VideoCameraItem.getSettings(recordingStack).aperture();
        FrameMeta meta = new FrameMeta(
                frameCount, 0f, 0f, 0f, stand.getYaw(), 0f, 0f, 0f, videoFov,
                ap, currentFocusDepth, depthGrid,
                pendingVpW, pendingVpH, pendingCropOffX, pendingCropOffY);
        ...
        ScreenshotRecorder.takeScreenshot(tripodFb, raw -> {
            if (raw == null) return;
            NativeImage cropped = cropTo16x9(raw);
            NativeImage frame   = boxDownsample(cropped, 1280);
            if (cropped != raw) cropped.close();
    ...
    // Classic (<1.21.11) version: TAIL hook after full player frame; no nested version guards.
    //? if <1.21.11 {
    public static void captureTripodFrame(RenderTickCounter tickCounter) {
        if (!recording || recordingArmorStandEntityId < 0) return;
        long now = System.currentTimeMillis();
